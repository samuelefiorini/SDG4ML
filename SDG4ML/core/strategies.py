...
def correlated(n=100, d=150, k=15, rho=0.5, amplitude=3.5, sigma=0.5, normalized=False, seed=None, **kwargs):
    ...
    where X ~ multi variate random Gaussian (0, THETA), THETA is a (d,d) matrix that has THETA[j,k] = rho^abs(j-k), beta is sparse and the nonzero values are in {+-amplitude}, noise ~ N(0,sigma).
...
def block_correlated(n=100, d=150, k=15, rho=0.5, amplitude=3.5, sigma=0.5, normalized=False, seed=None, **kwargs):
    """Generate a linear regression (X,Y) problem. The relationship between input and output is given by:

                                            Y = X*beta + noise

    where X is made by stacking on the rows a multi variate random Gaussian (0, THETA) of shape (n,k) with a single variate random Gaussian of shape (n,d-k), THETA is a (d,d) matrix that has THETA[j,k] = rho^abs(j-k), beta is sparse and the nonzero values are in {+-amplitude}, noise ~ N(0,sigma).
    ...
    Xleft = np.random.multivariate_normal(mean=np.zeros(k), cov=THETA, size=(n))
...
def zou_hastie_2005d(n=100, d=150, k=15, amplitude=3.5, sigma=0.5, normalized=False, seed=None, **kwargs):
    """Generate a linear regression problem (X,Y) as in Zou Hastie 2005 scenario (d).The relationship between input and output is given by:
     ...
     d : int, optional (default is `150`)
         total number of dimensions
     k : int, optional (default is `15`)
         total number of features with coefficient different from zero (k must be integer multiple of 3. If it is not, then k is forced to k - mod(k,3)).
     amplitude : float,  optional (default is `3.5`)
         amplitude of the generative linear model
     sigma : float, optional (default is `0.5`)
         Gaussian noise std
...
def wrapper(strategy='sparse', **kwargs):
    """Wrapper that let you generate a regression problem according to several strategies (see docstrings for further details).

    Parameters
    ----------
    strategy : str, optional (default is `sparse`)
        data generation strategy, this can be either 'null', 'sparse', 'correlated', 'block_correlated' or 'zou_hastie_2005d'.
    """
    if strategy.lower() == 'null':
        ...
        print("{} is not a valid strategy. Accepted strategies are: 'null', 'sparse', 'correlated', 'block_correlated' or 'zou_hastie_2005d'.")
